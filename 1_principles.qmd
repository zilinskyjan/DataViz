...
-   Conciseness vs. an "attention-grabbing" potential
...
library(tidyverse)
# Get fiscal data
imf <- read_csv("data/macro/imf-fiscalmonitor-apr2023.csv")
...
inf <- read_csv("data/macro/inflation_WDI.csv")
inf2022 <- inf %>% filter(year==2022)
econ2022 <- left_join(imf_wide2022,inf2022,by="country")
```

You may agree that you should almost never make graphs that look like this:
...
It turns out that whoever wrote the code decided to just drop a row (and not display Norway). Is that OK?
...
library(tidyverse)
imf <- read_csv("https://raw.githubusercontent.com/zilinskyjan/DataViz/main/data/macro/imf-fiscalmonitor-apr2023.csv")
```
...
Or you can highlight a subset of observations relevant for your analysis.
...
This would seem to suggest that higher (government) spending is not necessarily associated with faster price growth.

To be sure, a more careful analysis here would require looking it **changes** in government expenditures.

## Inflation magnitudes

Here are a few ways to show how sharply inflation increased between 2021 and 2022 in most OECD countries:
